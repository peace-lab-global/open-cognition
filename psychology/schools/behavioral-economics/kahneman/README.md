---
id: kahneman
title: "丹尼尔·卡尼曼 · Daniel Kahneman"
type: thinker
domain: psychology
school: behavioral-economics
era: contemporary
birth: 1934
death: 2024
tags:
  - 行为经济学
  - 双系统理论
  - 认知偏差
  - 前景理论
  - 启发式
  - 噪声
aliases:
  - 丹尼尔·卡尼曼
  - Daniel Kahneman
concepts:
  - system-1-2
  - heuristics-biases
  - prospect-theory
  - anchoring
  - availability
---

# 丹尼尔·卡尼曼 · Daniel Kahneman

## 基本信息 · Basic Information

| 项目 / Item | 内容 / Details |
|---|---|
| 生卒年 / Lifespan | 1934年3月5日 — 2024年3月27日 |
| 出生地 / Birthplace | 特拉维夫，巴勒斯坦托管地（今以色列）/ Tel Aviv, Mandatory Palestine |
| 国籍 / Nationality | 以色列裔美国 / Israeli-American |
| 教育 / Education | 希伯来大学学士与硕士；加州大学伯克利分校博士（1961）|
| 学术职位 / Positions | 希伯来大学心理学教授；不列颠哥伦比亚大学；普林斯顿大学Eugene Higgins心理学荣休教授 |
| 核心荣誉 / Honors | 2002年诺贝尔经济学奖（与弗农·史密斯共享）；2013年总统自由勋章 |
| 学派 / School | 行为经济学、认知心理学 |
| 核心合作者 / Collaborator | 阿莫斯·特沃斯基（Amos Tversky, 1937–1996）|

---

## 核心命题 · Core Theses

### 1. 双系统理论 · Dual-Process Theory

人类思维由两个截然不同的系统驱动：**系统1**（快速、自动化、直觉性、情绪化）和**系统2**（缓慢、刻意、分析性、费力）。大多数日常判断由系统1主导，系统2则充当"懒惰的监督者"。这一框架整合了数十年的认知心理学研究，成为理解人类决策的基础架构。

Human thinking is driven by two distinct systems: **System 1** (fast, automatic, intuitive, emotional) and **System 2** (slow, deliberate, analytical, effortful). Most everyday judgments are dominated by System 1, while System 2 acts as a "lazy overseer." This framework integrates decades of cognitive psychology research and provides the foundational architecture for understanding human decision-making.

### 2. 启发式与偏差 · Heuristics and Biases

在不确定条件下，人们依赖有限的几种启发式（认知捷径）进行判断，包括可得性启发、代表性启发和锚定效应。这些启发式通常高效且近似准确，但会产生**系统性、可预测的偏差**——这不是随机错误，而是人类认知的结构性特征。

Under uncertainty, people rely on a limited set of heuristics (cognitive shortcuts), including availability, representativeness, and anchoring. These heuristics are generally efficient and approximately accurate, but they produce **systematic, predictable biases** — not random errors, but structural features of human cognition.

### 3. 前景理论 · Prospect Theory

与传统经济学的期望效用理论不同，前景理论提出：人们评价结果时参照的是相对于**参考点**的收益与损失，而非绝对的财富水平；人们对**损失的敏感度约为收益的两倍**（损失厌恶）；概率不是线性加权的，而是通过决策权重函数进行非线性转换。

Unlike expected utility theory, prospect theory proposes that people evaluate outcomes relative to a **reference point** as gains and losses rather than absolute wealth levels; that **losses loom roughly twice as large as equivalent gains** (loss aversion); and that probabilities are not linearly weighted but transformed through a nonlinear decision-weighting function.

### 4. 锚定效应 · Anchoring Effect

人们在做出数值估计时，会过度受到最先接触到的信息（"锚"）的影响，即使这个锚与估计任务毫无关系。锚定效应是最稳健的认知偏差之一，在谈判、定价、法律判决等领域有广泛影响。

When making numerical estimates, people are excessively influenced by the first piece of information they encounter (the "anchor"), even when it is irrelevant to the estimation task. Anchoring is one of the most robust cognitive biases, with widespread implications for negotiation, pricing, and legal judgment.

### 5. 可得性启发 · Availability Heuristic

人们倾向于根据某类事件在记忆中的**容易提取程度**来判断其频率或概率。这导致近期发生的、情绪强烈的、媒体广泛报道的事件被过度估计，而统计上更重要但不显眼的事件被低估。

People tend to judge the frequency or probability of events by how **easily instances come to mind**. This leads to overestimation of recent, emotionally vivid, or media-covered events, and underestimation of statistically more significant but less salient events.

### 6. 噪声 · Noise

在卡尼曼晚年与西博尼、桑斯坦的合作中，他将关注点从**偏差**（系统性错误）转向**噪声**（判断中的随机变异）。不同的法官、医生、面试官面对同一案例时会给出截然不同的判断，这种变异既来自个体差异，也来自情境因素（如情绪、疲劳）。噪声虽然不如偏差引人注目，但造成的总体误差同样巨大。

In his later collaboration with Sibony and Sunstein, Kahneman shifted focus from **bias** (systematic error) to **noise** (random variability in judgments). Different judges, doctors, or interviewers give strikingly different assessments of the same case, driven by both individual differences and situational factors (mood, fatigue). While less visible than bias, noise produces aggregate errors of comparable magnitude.

---

## 思想脉络 · Intellectual Development

### 第一期（1955–1978）：认知心理学基础 · Foundations in Cognitive Psychology

卡尼曼在希伯来大学学习心理学，获学士与硕士学位。1961年在加州大学伯克利分校获博士学位，论文涉及视觉感知与注意力。回到希伯来大学后，他开始研究人类注意力的有限容量问题。1969年，他与阿莫斯·特沃斯基开始了改变心理学历史的合作——两人性格互补（卡尼曼偏向直觉与自我怀疑，特沃斯基偏向逻辑与自信），合作产出了大量开创性论文。1973–1974年，他们在《科学》杂志发表了关于启发式与偏差的系列论文，奠定了整个研究纲领的基础。

Kahneman studied psychology at the Hebrew University, earning bachelor's and master's degrees. He received his PhD from UC Berkeley in 1961, with a dissertation on visual perception and attention. Returning to the Hebrew University, he began investigating the limited capacity of human attention. In 1969, he began his historic collaboration with Amos Tversky — their complementary temperaments (Kahneman intuitive and self-doubting, Tversky logical and confident) produced an extraordinary stream of groundbreaking papers. In 1973–1974, their series of papers on heuristics and biases in *Science* laid the foundation for an entire research program.

### 第二期（1979–2002）：前景理论与行为经济学 · Prospect Theory and Behavioral Economics

1979年，卡尼曼与特沃斯基在《Econometrica》发表"前景理论"（Prospect Theory），直接挑战传统经济学的期望效用最大化假设。这一论文成为行为经济学的奠基之作。此后两人继续合作，提出框架效应、禀赋效应等重要概念。1996年特沃斯基因病去世后，卡尼曼继续深化研究。2002年，卡尼曼因"将心理学研究的洞见融入经济学科学"而获得诺贝尔经济学奖。

In 1979, Kahneman and Tversky published "Prospect Theory" in *Econometrica*, directly challenging the expected utility maximization assumption of traditional economics. This paper became the founding document of behavioral economics. The two continued to collaborate, developing framing effects, the endowment effect, and other key concepts. After Tversky's death from illness in 1996, Kahneman continued to deepen the research. In 2002, Kahneman received the Nobel Memorial Prize in Economic Sciences for "having integrated insights from psychological research into economic science."

### 第三期（2003–2024）：普及与拓展 · Popularization and Extension

2011年出版的《思考，快与慢》是卡尼曼学术生涯的系统总结，面向大众读者，将数十年研究成果整合在双系统框架下，成为全球畅销书。此后，他将研究兴趣拓展到幸福感测量（体验自我与记忆自我的区分）以及判断中的噪声问题。2021年与西博尼、桑斯坦合著出版《噪声》，进一步丰富了对人类判断错误的理解。卡尼曼于2024年3月27日逝世，享年90岁。

*Thinking, Fast and Slow* (2011) was the systematic summation of Kahneman's career, written for a general audience and integrating decades of research under the dual-system framework; it became a global bestseller. He subsequently expanded his interests to well-being measurement (the distinction between the experiencing self and the remembering self) and the problem of noise in judgment. *Noise* (2021), co-authored with Sibony and Sunstein, further enriched the understanding of human judgment error. Kahneman passed away on March 27, 2024, at the age of 90.

---

## 关键著作 · Key Works

| 年份 | 著作 | 说明 |
|---|---|---|
| 1973 | "On the Psychology of Prediction" (*Psychological Review*) | 与特沃斯基合著，研究代表性启发与基础率忽视 |
| 1974 | "Judgment under Uncertainty: Heuristics and Biases" (*Science*) | 启发式与偏差研究纲领的宣言式论文 |
| 1979 | "Prospect Theory: An Analysis of Decision under Risk" (*Econometrica*) | 前景理论，行为经济学奠基之作 |
| 1981 | "The Framing of Decisions and the Psychology of Choice" (*Science*) | 框架效应 |
| 1982 | *Judgment under Uncertainty: Heuristics and Biases* (编著) | 与特沃斯基、斯洛维奇合编的论文集 |
| 1999 | "Dimensions of Well-Being" | 体验效用与决策效用的区分 |
| 2003 | "Maps of Bounded Rationality" (*American Economic Review*) | 诺贝尔奖演讲论文 |
| 2011 | *Thinking, Fast and Slow*（《思考，快与慢》）| 系统总结双系统理论与认知偏差 |
| 2021 | *Noise: A Flaw in Human Judgment*（《噪声》）| 与西博尼、桑斯坦合著 |

---

## 重要概念 · Key Concepts

1. **系统1与系统2（System 1 and System 2）** — 快思考与慢思考的双系统模型。→ [详细条目](concepts/system-1-2.md)
2. **启发式与偏差（Heuristics and Biases）** — 认知捷径及其系统性错误。→ [详细条目](concepts/heuristics-biases.md)
3. **前景理论（Prospect Theory）** — 参考点依赖、损失厌恶、概率加权。→ [详细条目](concepts/prospect-theory.md)
4. **锚定效应（Anchoring Effect）** — 初始信息对后续判断的过度影响。→ [详细条目](concepts/anchoring.md)
5. **可得性启发（Availability Heuristic）** — 基于记忆易提取性的频率判断。→ [详细条目](concepts/availability.md)
6. **框架效应（Framing Effect）** — 同一问题的不同表述方式导致不同选择。
7. **损失厌恶（Loss Aversion）** — 损失带来的痛苦约为同等收益快乐的两倍。
8. **禀赋效应（Endowment Effect）** — 拥有某物后对其评价升高。
9. **基础率忽视（Base Rate Neglect）** — 判断时忽视先验概率信息。
10. **噪声（Noise）** — 判断中的随机变异，区别于系统性偏差。

---

## 思想坐标 · Intellectual Coordinates

- **承接 · Builds on**：赫伯特·西蒙（Herbert Simon）的有限理性（bounded rationality）理论；认知心理学的信息加工范式。Simon's bounded rationality; the information-processing paradigm of cognitive psychology.
- **批判 · Critiques**：传统经济学的理性人假设（Homo Economicus）；期望效用理论（von Neumann-Morgenstern）；贝叶斯推理作为人类判断的描述性模型。The rational agent assumption (Homo Economicus); expected utility theory; Bayesian reasoning as a descriptive model of human judgment.
- **同时代对话 · Contemporary dialogue**：理查德·塞勒（Richard Thaler）的"助推"理论；加里·吉仁泽（Gerd Gigerenzer）的"生态理性"争论；安东尼奥·达马西奥（Antonio Damasio）的躯体标记假说。Thaler's nudge theory; Gigerenzer's ecological rationality debate; Damasio's somatic marker hypothesis.
- **影响 · Influences**：行为经济学、行为金融学、公共政策设计、医疗决策科学、法律判决改革、人工智能人机交互设计。Behavioral economics, behavioral finance, public policy design, medical decision science, legal judgment reform, AI/HCI design.

---

## 当代应用 · Contemporary Applications

1. **公共政策助推 · Public Policy Nudging** — 英国行为洞察团队（BIT）、各国"助推单元"将卡尼曼的研究转化为默认选项设计、简化信息呈现等政策工具。The UK Behavioural Insights Team and national "nudge units" translate Kahneman's research into default-option design and simplified information presentation.

2. **金融投资决策 · Financial Investment** — 行为金融学直接继承了前景理论与偏差研究，帮助投资者识别过度自信、处置效应（过早卖出盈利股、过久持有亏损股）等系统性错误。Behavioral finance directly inherits prospect theory and bias research, helping investors identify overconfidence, the disposition effect, and other systematic errors.

3. **医疗诊断与决策 · Medical Diagnosis** — 噪声研究揭示了不同医生对同一患者诊断的巨大差异，推动了标准化诊断协议和决策辅助工具的开发。Noise research has revealed enormous diagnostic variation among doctors evaluating the same patient, driving development of standardized diagnostic protocols and decision aids.

4. **产品与用户体验设计 · Product and UX Design** — 框架效应、锚定效应和损失厌恶原则被广泛应用于定价策略、免费试用设计和用户引导流程。Framing effects, anchoring, and loss aversion principles are widely applied in pricing strategies, free-trial design, and user onboarding flows.

5. **司法判决改革 · Judicial Reform** — 噪声研究揭示了不同法官对相似案件量刑的巨大差异（"系统噪声"），推动了量刑指南和算法辅助判决的讨论。Noise research has revealed enormous sentencing disparities among judges for similar cases ("system noise"), driving discussions about sentencing guidelines and algorithmic decision aids.

6. **人工智能与人类判断 · AI and Human Judgment** — 卡尼曼的研究为"人在回路"（human-in-the-loop）系统设计提供了认知基础——理解人类判断的弱点，才能更好地设计人机协作。Kahneman's research provides the cognitive foundation for human-in-the-loop system design — understanding the weaknesses of human judgment enables better human-machine collaboration.

7. **教育与批判性思维 · Education and Critical Thinking** — 双系统框架被用于教学设计，帮助学生识别自身认知偏差，培养"去偏差"（debiasing）能力。The dual-system framework is used in instructional design to help students recognize their own cognitive biases and develop debiasing skills.

8. **幸福感研究 · Well-being Research** — 体验自我与记忆自我的区分影响了国民幸福指数（如不丹GNH、OECD美好生活指数）的测量方式。The experiencing-self/remembering-self distinction has influenced national well-being measurement (e.g., Bhutan's GNH, OECD Better Life Index).

---

## 常见误读 · Common Misreadings

### ❌ 误读一 · Misreading 1
**"系统1是坏的，系统2是好的。"** / "System 1 is bad, System 2 is good."

✅ **卡尼曼明确指出**：系统1是进化赋予人类的高效工具，在大多数日常情境中表现优异；系统2虽然更"理性"，但容量有限且容易疲劳。关键不在于消灭系统1，而在于识别系统1容易出错的情境并激活系统2。
Kahneman explicitly states that System 1 is an efficient evolutionary tool that performs well in most everyday situations. System 2, while more "rational," has limited capacity and fatigues easily. The key is not to eliminate System 1 but to recognize situations where it is error-prone and engage System 2.

### ❌ 误读二 · Misreading 2
**"人类总是非理性的。"** / "Humans are always irrational."

✅ **卡尼曼强调的是**：启发式在大多数情况下是有效的近似策略（"通常很好用"），只是在特定条件下产生系统性偏差。他反对的不是理性本身，而是对理性的**无条件假设**。吉仁泽的"生态理性"研究进一步证明了许多启发式在真实环境中的优越性。
Kahneman emphasizes that heuristics are effective approximation strategies in most situations ("usually good enough"), producing systematic biases only under specific conditions. He opposes not rationality itself but the **unconditional assumption** of rationality. Gigerenzer's "ecological rationality" research further demonstrates the superiority of many heuristics in real-world environments.

### ❌ 误读三 · Misreading 3
**"前景理论只是心理学理论，与经济学无关。"** / "Prospect theory is just a psychology theory, irrelevant to economics."

✅ **事实是**：前景理论直接挑战了冯·诺依曼-摩根斯坦的期望效用理论，催生了行为经济学这一整个学科分支，深刻改变了金融学、保险学和公共政策。2002年诺贝尔经济学奖本身就是对这一跨学科影响力的承认。
Prospect theory directly challenged von Neumann-Morgenstern expected utility theory, spawned the entire subfield of behavioral economics, and profoundly transformed finance, insurance, and public policy. The 2002 Nobel Prize in Economics itself acknowledges this cross-disciplinary impact.

### ❌ 误读四 · Misreading 4
**"卡尼曼和特沃斯基的理论可以被简单公式化。"** / "Kahneman and Tversky's theories can be easily formalized."

✅ **卡尼曼本人承认**：前景理论的价值函数和权重函数是描述性的而非规范性的，其参数随情境变化。理论的真正力量在于揭示人类决策的**复杂性**，而非提供简洁的数学模型。
Kahneman himself acknowledged that prospect theory's value function and weighting function are descriptive rather than prescriptive, with parameters that vary by context. The theory's true power lies in revealing the **complexity** of human decision-making, not in providing neat mathematical models.

---

## 东西方对话 · East-West Dialogue

### 唯识学 · Yogacara Buddhism
卡尼曼的"系统1自动化加工"与唯识学的**末那识**（第七识，恒审思量的自我执著）有深层共鸣：两者都揭示了意识表层之下持续运作的自动化认知过程。唯识学的"遍计所执性"（对事物的错误投射）可类比为系统1产生的系统性偏差。

Kahneman's "automatic System 1 processing" resonates deeply with Yogacara's **manas-vijnana** (the seventh consciousness, the persistent self-grasping): both reveal automated cognitive processes operating continuously beneath the surface of awareness. Yogacara's *parikalpita-svabhava* (imagined nature, erroneous projection onto things) can be analogized to the systematic biases produced by System 1.

### 庄子 · Zhuangzi
庄子"吾丧我"（《齐物论》）的境界可理解为一种超越系统1自动化认同和系统2刻意分析的**第三种认知模式**——一种非概念化的直接觉知。这与卡尼曼晚年对"专家直觉"（expert intuition）的讨论形成有趣对话：经过长期训练的专业人士（如棋手、消防员）能在系统1层面发展出高度准确的模式识别能力。

Zhuangzi's "losing the self" (*Qi Wu Lun*) can be understood as a **third mode of cognition** that transcends both System 1's automatic identification and System 2's deliberate analysis — a non-conceptual direct awareness. This forms an interesting dialogue with Kahneman's later discussion of "expert intuition": professionals with extensive training (chess players, firefighters) can develop highly accurate pattern recognition at the System 1 level.

### 佛教正念 · Buddhist Mindfulness
佛教正念修行的核心——**觉察当下的自动反应而不立即行动**——本质上是一种系统2对系统1反应的"元认知监控"训练。正念禅修者通过长期练习，能在刺激与反应之间创造一个"选择空间"，这与卡尼曼倡导的"去偏差"策略（在重要决策前刻意暂停）在结构上高度一致。

The core of Buddhist mindfulness — **observing automatic reactions without immediately acting on them** — is essentially a System 2 "metacognitive monitoring" training over System 1 responses. Through sustained practice, mindfulness meditators create a "choice space" between stimulus and response, structurally paralleling Kahneman's debiasing strategies (deliberately pausing before important decisions).

---

## 跨学科关联 · Cross-Disciplinary Connections

| 领域 / Domain | 关联方式 / Connection |
|---|---|
| 经济学 / Economics | 催生行为经济学，挑战理性人假设 |
| 金融学 / Finance | 行为金融学的理论基础（前景理论、过度自信） |
| 神经科学 / Neuroscience | 双系统理论的神经基础验证（fMRI研究） |
| 公共政策 / Public Policy | 助推理论、选择架构设计 |
| 医学 / Medicine | 临床决策中的偏差与噪声研究 |
| 法学 / Law | 司法判决中的噪声与偏差 |
| 人工智能 / AI | 人机协作中的认知偏差识别 |
| 哲学 / Philosophy | 对理性主义的修正，有限认识论 |

---

## 进阶阅读 · Further Reading

→ 详见 [reading-list.md](reading-list.md)

---

## 子条目索引 · Sub-Entry Index

| 条目 / Entry | 路径 / Path |
|---|---|
| 概念：系统1与系统2 | [concepts/system-1-2.md](concepts/system-1-2.md) |
| 概念：启发式与偏差 | [concepts/heuristics-biases.md](concepts/heuristics-biases.md) |
| 概念：前景理论 | [concepts/prospect-theory.md](concepts/prospect-theory.md) |
| 概念：锚定效应 | [concepts/anchoring.md](concepts/anchoring.md) |
| 概念：可得性启发 | [concepts/availability.md](concepts/availability.md) |
| 关键著作 | [works.md](works.md) |
| 生平年表 | [timeline.md](timeline.md) |
| 进阶阅读 | [reading-list.md](reading-list.md) |

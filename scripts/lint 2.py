...
REQUIRED_FRONTMATTER = {
    "thinker": ["id", "name", "type", "domain", "school", "era", "tags"],
...
def lint_file(path: Path, report: Report) -> None:
    ...
    # --- Cross-domain links (for thinker/concept) ---
    if entry_type in ("thinker", "concept"):
        domain = fm.get("domain", "")
        rel = path.relative_to(REPO_ROOT).as_posix()
        cross_pattern = f"../../../../domains/"
        if cross_pattern not in text:
            # Allow for same-domain cross-links too, but flag if literally none
            report.add(Finding(path, 1, "warn", "W005",
                               "no cross-domain link detected (expected `../../../../domains/...`)"))
